...
for To = 2:nFrames
    ...
    for m2=1:M-1
        [ign,srt]=sort([target_score{:,1}],'descend');
        m=srt(m2);
        for n2=m2+1:M
                n=srt(n2);
                    ...
                    if length(occSamples{n}(:,1))>1
                        for ind=2:length(occSamples{n})
                            if (overlap_ratio(occSamples{m}(1,:),occSamples{n}(ind,:))<0.2)
                                result(n,To,:)=occSamples{n}(ind,:);
                                feat_fc = mdnet_features_fcX(net_fc{n}, occFrames{n}(:,:,:,ind), opts);
                                feat_fc = squeeze(feat_fc)';
                                target_score{n}=feat_fc(1);
